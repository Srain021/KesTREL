# Quickstart — 在你的机器上启动 redteam-mcp
...
├── src/redteam_mcp/
│   ├── __init__.py
│   ├── __main__.py           # CLI 入口 (redteam-mcp 命令)
│   ├── server.py             # MCP 服务器
...
```powershell
# 1. CLI 能跑？
python -m redteam_mcp version
#   0.1.0

# 2. 诊断所有工具
python -m redteam_mcp doctor
#   ┌──────────┬─────────┬──────────────────┬──────────────┐
...
python -m redteam_mcp list-tools | more
...
* 每次 `call_tool` 写入 `~/.redteam-mcp/audit.log`
...
| 现象 | 原因 | 解决 |
|------|------|------|
| `AUTHORIZATION DENIED: authorized_scope is empty` | 没配 scope | `export REDTEAM_MCP_AUTHORIZED_SCOPE="*.lab"` |
| `ToolNotFoundError: 'nuclei' not found on PATH` | 二进制路径没配 | 在 `.env` 设 `REDTEAM_MCP_TOOL_NUCLEI=/path/to/nuclei.exe` |
...
| `tool.auth_denied` 日志 | 目标不在 scope 内 | 把目标加到 `REDTEAM_MCP_AUTHORIZED_SCOPE` |
